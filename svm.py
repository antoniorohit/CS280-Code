...
for MALIK in [True]: #, False]:
    ...
    if MALIK == True:
        if(os.path.isfile("./Results/shuffledData.p")):
            ...
            print('data successfully opened with pickle')
            print('opening labels..')
            shuffledLabels = pickle.load(open("./Results/shuffledLabels.p", 'rb'))
            print('labels successfully opened with pickle')
            first_type = 28
        else:
            n_bins=9
            ind=0
...
#                 ori_7_hist = np.float64(ori_7_hist)/(np.linalg.norm(ori_7_hist))
    ...
    for elem in training_Size:
        ...
        for i in range(elem):
            for j in range(elem):
                mat_train[i][j]= my_kernel(X_train[i], X_train[j], first_type)
        ...
        for i in range(test_size):
            for j in range(elem):
                mat_test[i][j]= my_kernel(X_test[i], X_train[j], first_type)
...
#     ax.plot(training_Size, linear_errorRate_array)
