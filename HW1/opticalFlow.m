...
for i = 1:M/scale
    for j = 1:M/scale
            u_train(i,j) = (-train_vel/train_scene(i*scale,j*scale));
            v_train(i,j) = 0;
    end
end
...
for i = 1:M
    for j = 1:M
            if i > M*k_road % wall
                v_car(i,j) = (car_vel*(i-M*k_road)/train_scene(i*scale,j*scale));
                u_car(i,j) = (car_vel*(j-M/2)/train_scene(i*scale,j*scale));
            else % road
                v_car(i,j) = -(car_vel*(car_ht)/train_scene(i*scale,j*scale));
                u_car(i,j) = (car_vel*(j-M/2)/train_scene(i*scale,j*scale));                
            end
    end
end
